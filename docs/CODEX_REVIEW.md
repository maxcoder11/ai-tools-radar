...
> **本文档是 v1.0.0 的工程记录（已归档）。** 8 轮外审在 v1.0.0 收敛（最后两轮 0 个 P1），
> 逐轮的 finding、修法与实测都留在下面。接手 `outreach/` 前值得扫一眼——不是为了看结论，
> 是为了看**错法**：反复出现的两类是「这个防御的代码路径真的会被走到吗」和
> 「同一个 bug 在别处是不是已经修过了」；第三类是「动手写新实现前先 grep 仓库里有没有
> 已经写对的」（文件锁就是 `creds.mjs` 已经写对了，我又重写了一遍错的）。
> 下一轮评审请把「评审对象」改成新的提交范围。
...
R13 按 Codex 点名的状态(`manual`/`skipped_*`)补了 `CLAIM_BLOCKING`,交付前我自己穷举
了一遍"从 success 出发遍历全部状态",发现 **`email_verified` 和 `draft` 仍会撤标记** ——
已投达的域照样复活。也就是说我只补了实例,没修根因。

根因是**"哪些状态该撤标记"这个问题本身就不该存在**。实测证明:认领会把状态写成
`delivery_ambiguous`,而守卫拒绝它迁到任何可重试态(`blocked`/`failed`/`email_verified`/
`draft` 四个全被拦)—— **有标记的域永远走不到"需要回池"的状态**,自动撤销从来没有
用武之地;真正需要回池的域(如 `blocked → email_verified`)压根没有过标记。

所以删掉自动撤销,改成显式的 `releaseClaim()`,只由人工裁决调用。
穷举复验:遍历全部 12 个状态,**没有一个能让已投达的域复活**。

顺带确认了两道闸是独立的(只撤标记不会放行)。

**但随后被用户指出:我把这写成了"人工裁决的两步流程"是错的。**
`delivery_ambiguous` 的语义就是"可能已经投出去了",把它放回池 = 可能给站方发两份 ——
这恰恰是整套设计要防的事,而我给了可复制的命令教人怎么放回去。
现在 guidance 只说一件事:**去站上看链接在不在**。在 → 标 `success`
(`verify_link --pending --update-status` 本来就会自动做);不在或看不出来 → **什么都别动**。
`releaseClaim()` 保留为逃生阀,但不写进流程。

冒烟相应改成**穷举断言**(不再逐个状态列举 —— 列举正是漏掉的原因),
并新增"认领后走不到可重试态"这条**前提断言**:一旦这个前提塌了,
"不需要自动撤标记"的推理就不成立,测试会立刻红。
配置对拍也加了 7 组 py/js 共享常量比对(`STATUSES`/`CLAIM_BLOCKING`/`REGRESSIVE` …),
补上我文档里一直标着"手工对齐、没有对拍"的那个缺口。
...
2. **锁只留给状态投影的读改写**,并且:
   ...
   - 陈旧接管**保留但简化成"超时直接覆盖"**:持有者已死 → 30s 接管;
     ...
     **任何被遗弃的锁都自动回收,永远不需要人工 `rm`。**
     兜底线取 10 分钟的依据是实测(见 `LOCK_HARD_MS` 注释):临界区只做本地文件
     读+追加,100 万行/316MB 的账本也只要 4.0s,余量 150 倍;

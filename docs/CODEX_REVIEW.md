...
| R9 | Claude | 按 R8 清单全修 + 逐条实测 ← **本文档写的是这之后的状态** |
...
### 6.5 看门狗强退时的 `SIGKILL`(R6 已给出结论,保留备忘)
